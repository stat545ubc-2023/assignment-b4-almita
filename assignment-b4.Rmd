...
So the book actually starts at line 70 and ends at line 7385, I'll extract the relevant content to work with it. I'll also keep lines that have at least 1 character. 
...
head(frankenstein)
```
Now I need to create the unique words list. 
...
```
And then I need to count the number of unique words and remove the stop words so I can create the plots. For removing the stopwords I used the [tm: text mining](https://cran.r-project.org/web/packages/tm/index.html) package

```{r}
frankenstein_unique <- as_tibble(frankenstein_words) %>%
	filter(!value %in% tm::stopwords("SMART")) 
	group_by(value) %>%
	reframe(count = n()) %>%
	rename(word = value) 
...
And I'll also make another plot, this time a barplot, to show the top 15 words:
